        ...
        @if (Route::has('login'))
            <div class="hidden fixed top-0 right-0 px-6 py-4 sm:block">
                ...
                @else
                    ...
                    @endif
                @endauth
            <h1>{{ Cookie::get('lang') }}</h1>
            <h1>{{ getLang() }}</h1>
                {{-- <h1>{{ app()->getLocale() }}</h1> --}}
                <div class="card-tools mt-2">
                    <select class="form-select lang" id="lan" aria-label="" data-url="{{ route('setLan') }}">
                        <option {{ getLang() == 'en' ? 'selected' : '' }} value="en">Eng</option>
                        <option {{ getLang() == 'bn' ? 'selected' : '' }} value="bn">Ban</option>
                    </select>
                </div>
                <div>
                    <a href="{{ langRoute('allNews') }}" class="btn btn-info">All News</a>
                </div>
            </div>
